...
export default function RegisterPage() {
  const { registerWithEmail, loginWithGoogle, authLoading, authError, setAuthError } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 selection:bg-secondary-fixed selection:text-on-secondary-fixed">
      {/* Ambient glow */}
      <div className="absolute top-1/4 -right-20 w-80 h-80 rounded-full bg-secondary-fixed opacity-30 blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-1/4 -left-20 w-80 h-80 rounded-full bg-tertiary-fixed opacity-20 blur-3xl pointer-events-none"></div>
      ...
      <div className="relative w-full max-w-md">
        ...
        {/* Card */}
        <div className="p-8 rounded-2xl bg-surface-container-lowest border border-surface-container-high shadow-xl space-y-5">
          <div className="text-center space-y-1">
            <h1 className="font-display text-2xl font-bold text-primary">
              Create Student Account
            </h1>
            <p className="text-xs text-on-surface-variant">
              Match your verified coursework and projects to high-impact internships.
            </p>
          </div>

          {/* Student-Only Notice */}
          <div className="p-3 rounded-xl bg-blue-50 border border-blue-200 text-secondary text-[11px] flex items-start gap-2">
            <span className="material-symbols-outlined text-[16px] text-secondary mt-0.5 flex-shrink-0">
              verified
            </span>
            <span>
              <strong>Student-Only Platform:</strong> Free for verified undergraduate and graduate engineering students.
            </span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
            ...
            <div>
              ...
              <input
                ...
                placeholder="aanya@university.edu or personal"
                className="w-full px-3.5 py-2.5 rounded-xl bg-surface border border-outline-variant text-on-surface text-xs focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/15 transition-all"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={authLoading || submitting}
              className="w-full mt-2 py-3 rounded-xl bg-secondary text-on-secondary font-bold text-xs hover:bg-secondary-container transition-all shadow-md flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {submitting ? (
                <>
